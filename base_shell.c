...
int find_builtin(info_t *info)
{
	...
	for (i = 0; builtintbl[i].type; i++)
		if (strs_comp(info->argv[0], builtintbl[i].type) == 0)
		{
			info->line_count++;
...
void find_cmd(info_t *info)
{
	...
	path = which_pth(info, get_environment_variable(info, "PATH="), info->argv[0]);
	if (path)
	{
		info->path = path;
		fork_cmd(info);
	...
	else
	{
		if ((is_shell_interactive(info) || get_environment_variable(info, "PATH=")
			|| info->argv[0][0] == '/') && if_excute(info, info->argv[0]))
			fork_cmd(info);
		else if (*(info->arg) != '\n')
		{
			info->status = 127;
